...
export class AuditsTabComponent extends BaseComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  loadUsersActivities(filters: any = null): void {
    ...
    this.httpService.GET(UsersController.GetUsersAuditsForAdmins(this.user.id), this.currentFilters)
      .subscribe(res => {
        console.log(res);
  ...
  loadUsersActivitiesForFirstTime(filters: any = null): void {
    this.httpService.GET(UsersController.GetUsersAuditsForAdmins(this.user.id), filters)
      .subscribe(res => {
        console.log(res);
