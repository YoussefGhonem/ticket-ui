...
export class EventFiltersComponent extends BaseComponent implements OnInit {
  ...
  private getStatusForAdmin() {
    this.statusForAdmin = Object.keys(EventStatusEnum).map(id => ({ id, name: EventStatusEnum[id] })).slice(0, 4).sort((a, b) => a.name.localeCompare(b.name));
  ...
  export() {
    ...
    this.httpService.ExportExcel(EventsController.ExportToExcel, body)
  ...
  onClick(event) {
    console.log(event.target.tagName.toLowerCase().trim());
    if (event.target.classList.contains('ng-option') || event.target.tagName.toLowerCase().trim() == 'b' || event.target.tagName.toLowerCase().trim() == 'i' || event.target.tagName.toLowerCase().trim() == 'span')
      this.testBoolean = false;
