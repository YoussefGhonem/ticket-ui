...
export class AuditsTabComponent extends BaseComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  loadUsersActivities(filters: any = null): void {
    ...
    this.httpService.GET(UsersController.GetUsersAuditsForVendor(this.user?.committeeMember.userId), this.currentFilters)
      .subscribe(res => {
        console.log(res);
  ...
  loadUsersActivitiesForFirstTime(filters: any = null): void {
    this.httpService.GET(UsersController.GetUsersAuditsForVendor(this.user?.committeeMember?.userId), filters)
      .subscribe(res => {
        console.log(res);
